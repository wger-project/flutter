...
import 'package:wger/features/routines/widgets/gym_mode/log_page/palette.dart';
...
class _LogPageState extends ConsumerState<LogPage> {
  ...
  void _ensureControllersForSlot(SlotPageEntry sp, GymModeState gymState) {
    ...
    final log = Log.fromSetConfigData(config);
    log.routineId = gymState.routine.id;
    log.iteration = gymState.iteration;
    log.id = sp.logId;
    // What the user already logged wins over the routine target, so a set keeps
    // its values when the page is disposed and rebuilt.
    log.weightUnitId = sp.loggedWeightUnitId ?? _selectedWeightUnitId;
    log.weight = sp.loggedWeight ?? log.weight;
    log.repetitions = sp.loggedReps ?? log.repetitions;
    log.rir = sp.loggedRir ?? log.rir;
    _logs[sp.uuid] = log;

    _weightControllers[sp.uuid] = TextEditingController(
      text: formatSetValue(sp.loggedWeight ?? config.weight),
    );
    _repsControllers[sp.uuid] = TextEditingController(
      text: formatSetValue(sp.loggedReps ?? config.repetitions),
    );
    _rirControllers[sp.uuid] = TextEditingController(
      text: formatSetValue(sp.loggedRir ?? config.rir),
    );
  }
  ...
  void _initWeightUnit() {
    ...
    if (exerciseId != null) {
      final priorLogs =
          gymState.routine
              ...
              .where((l) => l.date.isBefore(gymState.workoutStart) && l.weightUnitId != null)
              .toList()
    ...
    // A unit already used for a set logged in this session wins: it is the
    // choice the user made on this page before it was disposed and rebuilt.
    final sessionUnit = widget._pageEntry.slotPages
        .firstWhereOrNull((sp) => sp.loggedWeightUnitId != null)
        ?.loggedWeightUnitId;

    var unitId =
        pickToggleUnit(sessionUnit) ??
        pickToggleUnit(priorUnit) ??
        pickToggleUnit(firstSlot?.setConfigData?.weightUnitId);
    ...
    for (final entry in _logs.entries) {
      // Never restamp a set that was already logged — its unit is whatever it
      // was actually logged in.
      if (_loggedUnitFor(entry.key) == null) {
        entry.value.weightUnitId = _selectedWeightUnitId;
      }
    }
  }

  /// The unit a set was logged in, or null while it is still pending.
  int? _loggedUnitFor(String slotUUID) =>
      ref.read(gymStateProvider).getSlotPageByUUID(slotUUID)?.loggedWeightUnitId;
  ...
  /// Switches the page between kg and lb (FR4c).
  ///
  /// Sets that have already been logged keep both their value and their unit —
  /// a set logged at 30 kg must never re-render as "30 lbs". Only the pending
  /// sets follow the toggle, and their numbers are *converted*, so what the
  /// input field shows is what gets saved.
  void _toggleWeightUnit() {
    final oldUnitId = _selectedWeightUnitId;
    final newUnitId = oldUnitId == WEIGHT_UNIT_KG ? WEIGHT_UNIT_LB : WEIGHT_UNIT_KG;
    final gymState = ref.read(gymStateProvider);

    setState(() {
      _selectedWeightUnitId = newUnitId;
      for (final entry in _logs.entries) {
        final slotPage = gymState.getSlotPageByUUID(entry.key);
        if (slotPage?.loggedWeightUnitId != null) {
          continue;
        }
        entry.value.weightUnitId = newUnitId;

        final controller = _weightControllers[entry.key];
        final current = num.tryParse(controller?.text.replaceAll(',', '.') ?? '');
        if (controller == null || current == null) {
          continue;
        }
        controller.text = formatSetValue(
          convertWeight(
            current,
            from: oldUnitId,
            to: newUnitId,
            rounding: slotPage?.setConfigData?.weightRounding,
          ),
        );
      }
    });
  ...
  Future<void> _onSetChecked(
    ...
    try {
      ...
      await ref.read(workoutLogProvider).addEntry(log, dayId: ref.read(gymStateProvider).dayId);
      // Hand the values to the gym state: the log page's own State dies with
      // the page, the state does not.
      gymNotifier.markSlotPageAsDone(
        uuid,
        isDone: true,
        weight: log.weight,
  ...
  Widget _buildActionChips(
    ...
    void showHistory() {
      ...
      showModalBottomSheet(
        ...
        builder: (_) => Consumer(
          builder: (ctx, ref, _) {
            // Past logs come straight from the local DB (not the gym-mode
            // routine snapshot) so the log scope settings apply and no routine
            // hydration is needed.
            ...
            final sessions = _historySessionsFor(gymState, pastLogs.value ?? const []);
  ...
  void _showTypePickerSheet(BuildContext context, SlotPageEntry sp, PageEntry pageEntry) {
    final currentType = resolveType(sp);
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      builder: (_) => _TypePickerSheet(
        ...
        onPick: (type) {
          ...
          ref.read(gymStateProvider.notifier).setSlotTypeOverride(sp.uuid, type);
        },
        onRemove: () {
          Navigator.of(context).pop();
  ...
  Widget build(BuildContext context) {
    ...
    final activeExercise =
        activeSlot?.setConfigData?.exerciseOrNull ??
    ...
    for (final sp in logSlotPages) {
      if (resolveType(sp) == SlotEntryType.normal) {
