...
class _SetsSection extends StatelessWidget {
  ...
  /// Unit the page is currently set to. Only pending rows follow it; a logged
  /// row stays pinned to [SlotPageEntry.loggedWeightUnitId].
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final type = resolveType(sp);
                final badgeLabel = type == SlotEntryType.normal
                    ? '${workingCounts[sp.uuid] ?? index + 1}'
                    : typeBadgeChar(type, AppLocalizations.of(context));

                // A logged set renders what was actually logged, in the unit it
                // was logged in. A pending one mirrors its input controller so
                // typing previews live, and follows the page's unit toggle.
                final loggedWeight = formatSetValue(sp.loggedWeight);
                final loggedReps = formatSetValue(sp.loggedReps);
                final rowUnitId = sp.loggedWeightUnitId ?? weightUnitId;
                final unitLabel = rowUnitId == WEIGHT_UNIT_KG ? 'kg' : 'lbs';

                final weightText = isDone
                    ? (loggedWeight.isNotEmpty ? loggedWeight : '—')
                    : (weightControllers[sp.uuid]?.text.isNotEmpty == true
                          ? weightControllers[sp.uuid]!.text
                          : '—');
                final repsText = isDone
                    ? (loggedReps.isNotEmpty ? loggedReps : '—')
                    : (repsControllers[sp.uuid]?.text.isNotEmpty == true
                ...
                return Column(
                  ...
                  children: [
                    Padding(
                      ...
                      child: Text(
                        sp.setConfigData?.exerciseOrNull?.getTranslation(languageCode).name ?? '',
                        style: TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.w700,
                          letterSpacing: 0.03,
