...
class _GymLogHeaderState extends ConsumerState<_GymLogHeader> {
  ...
  Widget build(BuildContext context) {
    ...
    final totalWorkoutSeconds = gymState.isInitialized
        ? DateTime.now().difference(gymState.workoutStart).inSeconds
        : 0;
...
class _QueueChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // A superset page names every exercise it holds ("Row + Squat"), so the
    // queue is readable without opening the page. Each name is shortened
    // *before* joining — truncating the composed string would clip the second
    // name mid-word.
    final names = page.exercises.map((e) => e.getTranslation(languageCode).name).toList();
    final maxLen = names.length > 1 ? 12 : 14;
    final shortName = names.isEmpty
        ? AppLocalizations.of(context).exerciseNr('${index + 1}')
        : names.map((n) => n.length > maxLen ? '${n.substring(0, maxLen - 1)}…' : n).join(' + ');
