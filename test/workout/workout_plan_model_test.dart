...
void main() {
  group('model tests', () {
    test('Test the filterLogsByExercise method', () {
      ...
      final logExercise1 =
          workout.filterLogsByExerciseBase(getTestExerciseBases()[0]);
      expect(logExercise1.length, 2);
      expect(logExercise1[0].id, 1);
      expect(logExercise1[1].id, 2);

      final logExercise2 =
          workout.filterLogsByExerciseBase(getTestExerciseBases()[1]);
      expect(logExercise2.length, 1);
      expect(logExercise2[0].id, 3);

      expect(workout.filterLogsByExerciseBase(getTestExerciseBases()[2]).length,
          0);
    });
  });
}
