...
void main() {
  ...
  group('image management', () {
    ...
    test('stores an image with all license fields set', () {
      final file = File('/tmp/licensed.jpg');

      container.read(addExerciseProvider.notifier).addExerciseImages([
      ...
      final images = container.read(addExerciseProvider).exerciseImages;
      expect(images.length, 1);
      expect(images.first.imageFile.path, file.path);
    });

    test('accepts an image with only some metadata fields set', () {
      container.read(addExerciseProvider.notifier).addExerciseImages([
        ExerciseSubmissionImage(
          imageFile: File('/tmp/partial.jpg'),
          title: 'Only Title',
          type: ImageType.threeD,
        ),
      ]);

      expect(container.read(addExerciseProvider).exerciseImages.length, 1);
    });

    test('addExerciseImages with an empty list leaves the state empty', () {
      container.read(addExerciseProvider.notifier).addExerciseImages([]);

      expect(container.read(addExerciseProvider).exerciseImages, isEmpty);
    });

    test('clear on an empty image list is a no-op', () {
      container.read(addExerciseProvider.notifier).clear();

      expect(container.read(addExerciseProvider).exerciseImages, isEmpty);
    });

    test('accepts very long metadata strings', () {
      final longString = 'a' * 1000;

      container.read(addExerciseProvider.notifier).addExerciseImages([
        ExerciseSubmissionImage(
          imageFile: File('/tmp/long.jpg'),
          title: longString,
          author: longString,
    ...
    test('accepts special characters in metadata', () {
      container.read(addExerciseProvider.notifier).addExerciseImages([
        ExerciseSubmissionImage(
          imageFile: File('/tmp/special.jpg'),
