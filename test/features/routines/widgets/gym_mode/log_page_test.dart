...
class _FakeLogMutations implements WorkoutLogMutations {
  ...
  final List<int?> addedDayIds = [];

  @override
  Future<void> addEntry(Log log, {int? dayId}) async {
    added.add(log);
    addedDayIds.add(dayId);
  }
...
void main() {
  ...
  group('LogPage core journeys', () {
    ...
    testWidgets('logging a set persists it and marks the set done (FR1a)', (tester) async {
      ...
      expect(fakeLogs.added.single.routineId, gymState.routine.id);
      expect(fakeLogs.added.single.iteration, gymState.iteration);
      // The lazy session needs the day, otherwise days that need logs to
      // advance can't see it (issue wger#2460).
      expect(fakeLogs.addedDayIds.single, gymState.dayId);
    ...
    // Regression: everything the user logged used to live only in the LogPage's
    // widget State, so the PageView disposing the page wiped it. The row still
    // read DONE (that flag lives in the keep-alive provider) but the weight came
    // back as the routine target — "—" for a set with no prescribed weight.
    testWidgets('logged values survive the page being disposed and rebuilt (FR-persist)', (
      tester,
    ) async {
      final page = firstSetPage();
      final firstSlotUuid = logSlots(page).first.uuid;

      // A set with no prescribed weight is the case that used to blank out:
      // there is no target to fall back on, so the loss was total.
      logSlots(page).first.setConfigData!.weight = null;

      await pumpLogPage(tester);

      await tester.enterText(find.byKey(const ValueKey('gym-input-weight')), '82.5');
      await tester.enterText(find.byKey(const ValueKey('gym-input-reps')), '7');
      await tester.tap(find.byKey(const ValueKey('gym-log-set-button')));
      await tester.pumpAndSettle();

      // The values are in the gym state, not just in the widget.
      final slot = reread(page).slotPages.firstWhere((sp) => sp.uuid == firstSlotUuid);
      expect(slot.logDone, isTrue);
      expect(slot.loggedWeight, 82.5);
      expect(slot.loggedReps, 7);

      // Tear the page down completely and build a fresh one, as navigating to
      // another exercise and back does.
      ...
      expect(
        find.descendant(
          of: find.byKey(ValueKey('gym-set-row-$firstSlotUuid')),
          matching: find.text('82.5'),
        ),
        findsOneWidget,
      );
      expect(
    ...
    // FR4c: switching units converts what is still to be logged, and never
    // relabels a set that is already in the books.
    testWidgets('the kg/lb toggle converts pending values and pins logged ones', (tester) async {
      final page = firstSetPage();
      final firstSlotUuid = logSlots(page).first.uuid;
      await pumpLogPage(tester);

      // Log the first set at 30 kg.
      await tester.enterText(find.byKey(const ValueKey('gym-input-weight')), '30');
      await tester.tap(find.byKey(const ValueKey('gym-log-set-button')));
      await tester.pumpAndSettle();
      expect(
        reread(page).slotPages.firstWhere((sp) => sp.uuid == firstSlotUuid).loggedWeightUnitId,
        WEIGHT_UNIT_KG,
      );

      // The next (pending) set is seeded from its 100 kg target.
      final weightBefore = tester.widget<TextField>(
        find.byKey(const ValueKey('gym-input-weight')),
      );
      expect(weightBefore.controller!.text, '100');

      await tester.tap(find.byKey(const ValueKey('gym-unit-toggle')));
      await tester.pumpAndSettle();

      // 100 kg ≈ 220.46 lb, snapped to the nearest half unit.
      final weightAfter = tester.widget<TextField>(find.byKey(const ValueKey('gym-input-weight')));
      expect(weightAfter.controller!.text, '220.5');

      // The already-logged set keeps both its number and its unit.
      final loggedRow = find.byKey(ValueKey('gym-set-row-$firstSlotUuid'));
      expect(find.descendant(of: loggedRow, matching: find.text('30')), findsOneWidget);
      expect(find.descendant(of: loggedRow, matching: find.text(' kg')), findsOneWidget);
