...
void main() {
  ...
  testWidgets(
    'swiping back from the first exercise cannot reach the start page',
    (WidgetTester tester) async {
      // Regression: the start page is permanently child 0 of the PageView, so a
      // backward swipe used to escape a session in progress and re-offer "start".
      await tester.pumpWidget(renderGymMode());
      await tester.pumpAndSettle();
      await tester.tap(find.byType(TextButton));
      await tester.pumpAndSettle();
      ...
      // Drag hard in the other direction.
      await tester.drag(find.byType(LogPage), const Offset(600.0, 0.0));
      await tester.pumpAndSettle();
  ...
  testWidgets('offline summary shows the local session stats', (WidgetTester tester) async {
    ...
    await withClock(Clock.fixed(DateTime(2021, 5, 1, 14, 33)), () async {
      ...
      // Swipe to the first log page, jump to the session page via Finish,
      // then swipe on to the summary.
      await tester.drag(find.byType(StartPage), const Offset(-500.0, 0.0));
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const ValueKey('gym-finish-button')));
      await tester.pumpAndSettle();
